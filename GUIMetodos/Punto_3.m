...
% Last Modified by GUIDE v2.5 24-Sep-2016 17:04:51
        ...
        if Mtdo==1
            ...
            if Tp == 1
                ...
                if cri == 2
                    ...
                    plot(ind70(1),ind70(2),'Om');
                    plot(ind90(1),ind90(2),'Ob');
                    legend('Señal ingresada','2%','70%','90%')
                else
                    plot(ind5(1),ind5(2),'Ok');
                    plot(ind70(1),ind70(2),'Om');
                    plot(ind90(1),ind90(2),'Ob');
                    legend('Señal ingresada','5%','70%','90%')
                end
            end
...
Gm=GM;
 ...
 num=strcat(char(vpa(poly2sym(Gm.num{1,1},s),3)),'*exp(-',num2str(Gm.OutputDelay),'*s)');
 deno=strcat(char(vpa(poly2sym(Gm.den{1,1},s),3)));
%num='hola';deno='como estas';
fileID = fopen('Ecuacion.txt','w');
formatSpec = 'La respuesta obtenida es:\n  %s \n-----------\n %s';
fprintf(fileID,formatSpec,num,deno);
fclose(fileID);

fileID = fopen('Aprox.txt','w');
...
a='Jahanmiri';
b='Primer orden de smith';
c='Modelo de primer orden Ho';
d='Modelo de segundo mas tiempo muerto orden Ho';
e='Modelo tangente';
if(Vmin==1)
f=a;
plot(ts,Gm11,'m','linewidth',2,'parent',handles.Signal);hold on
GM=Gm1;
legend(a)
end;
if(Vmin==2)
f=b;
plot(ts,Gm22,'m','linewidth',2,'parent',handles.Signal);hold on
GM=Gm2;
end;
if(Vmin==3)
f=c;
plot(ts,Gm33,'m','linewidth',2,'parent',handles.Signal);hold on
GM=Gm3;
end;
if(Vmin==4)
f=d;
plot(ts,Gm44,'m','linewidth',2,'parent',handles.Signal);hold on
GM=Gm4;
end;
if(Vmin==5)
f=e;
if Tp == 0
plot(ts,Gm55,'m','linewidth',2,'parent',handles.Signal);hold on
GM=Gm5;
end
end;
if Tp == 1
 a='Magenta - Aproximado Jahanmiri';
 b='Negro - Respuesta Ingresado ';
 c='El mejor método es:';
 xo={a,b,c,f};   
else
 a='Modelos aproximados:';   
 b='Magenta - Jahanmiri';
 d='Magenta(:)-Primer Orden De Smith';
 e='Azul - Modelo De Primer OrdenHo';
 g='Verde -Segundo Mas Tiempo Muerto Orden Ho';
 h='Amarillo - Tangente';
 ttt='Negro - Respuesta Ingresado ';
 ccc='El  método es:';
 xo={a,b,c,d,e,g,h,ttt,ccc,f};
end
...
b1='Señal sin huecos ';
c1='Señal suavizada';
d1='Señal suavizada Vs Original';
aa={a1;b1;c1;d1};
